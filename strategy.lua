...
function M.mutate(princessSlot, droneSlot, targetSpecies, mutation)--单步突变
    ...
    --1.校验输入
    if not bot.inventory[princessSlot] or bot.inventory[princessSlot].type ~= "beePrincess"
        or not bot.inventory[droneSlot] or bot.inventory[droneSlot].type ~= "beeDrone"
        or type(targetSpecies) ~= "string" or type(mutation) ~= "table" or type(mutation.parents) ~= "table" then
        error(string.format("错误的调用strategy.mutate(%s, %s, %s)", tostring(princessSlot), tostring(droneSlot), tostring(targetSpecies)))
    end
    ...
    local pSpecies1, pSpecies2 = bot.inventory[princessSlot].species[1], bot.inventory[princessSlot].species[2]
    ...
    --2.执行突变(自动突变分支)
        ...
        while true do
            ...
            if pSpecies1 == pSpecies2 then
                if pSpecies1 == mutation.parents[1] then
                    local droneSlot = allele22[1] or allele12[1]
                    if droneSlot then
                        nextGeneration(droneSlot)
                        pSpecies1, pSpecies2 = bot.inventory[princessSlot].species[1], bot.inventory[princessSlot].species[2]
                    else
                        bot.inventoryLabel = previousLabel
                        return nil, princessSlot
                    end
                elseif pSpecies1 == mutation.parents[2] then
            ...
            elseif (pSpecies1 == mutation.parents[1] and pSpecies2 == mutation.parents[2]) or (pSpecies1 == mutation.parents[2] and pSpecies2 == mutation.parents[1]) then
...
function M.purify(princessSlot, droneSlot, targetGenes, assistantDroneSlot, labelSuffix)--纯化
    if not bot.inventory[princessSlot] or bot.inventory[princessSlot].type ~= "beePrincess"
        or not bot.inventory[droneSlot] or bot.inventory[droneSlot].type ~= "beeDrone"
        or not bot.inventory[assistantDroneSlot] or bot.inventory[assistantDroneSlot].type ~= "beeDrone"
        or type(targetGenes) ~= "table" or type(targetGenes.species) ~= "string" then
        error(string.format("错误的调用strategy.purify(%s, %s, %s)", tostring(princessSlot), tostring(droneSlot), tostring(assistantDroneSlot)))
    end
    ...
    local function getDrones(highFertilityOnly, includeAssistant)
        ...
        if includeAssistant and bot.inventory[assistantDroneSlot] then
            local isPresent = false
            for _, slot in pairs(result) do
                if slot == assistantDroneSlot then
                    isPresent = true
                    break
                end
            end
...
function M.breedDrones(princessSlot, droneSlot, targetAmount)--繁殖雄蜂
    if not bot.inventory[princessSlot] or bot.inventory[princessSlot].type ~= "beePrincess"
        or not bot.inventory[droneSlot] or bot.inventory[droneSlot].type ~= "beeDrone"
        or type(targetAmount) ~= "number" or targetAmount < 1 then
        error(string.format("错误的调用strategy.breedDrones(%s, %s, %s)", tostring(princessSlot), tostring(droneSlot), tostring(targetAmount)))
    end
    for _, chromosome in pairs(chromosomeList) do
        local p1, p2 = bot.inventory[princessSlot][chromosome][1], bot.inventory[princessSlot][chromosome][2]
        local d1, d2 = bot.inventory[droneSlot][chromosome][1], bot.inventory[droneSlot][chromosome][2]
...
function M.task(species)--制定突变链
    ...
    --计算突变路径
    print("计算突变路径")
    local targetAlreadyExists = beeData.getDroneTag(species) ~= nil
